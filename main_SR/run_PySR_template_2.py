...
def main(args):
    ...
    custom_loss = """
        function expr_loss(ex::AbstractExpression, dataset::Dataset, options)
            ...
            num_negative_constants = count(is_negative_constant, get_tree(q)) + count(is_negative_constant, get_tree(m)) + count(is_negative_constant, get_tree(g)) + count(is_negative_constant, get_tree(h))

            if num_negative_constants > 0
                return 1000 * num_negative_constants
            end

            g_power_constraint(node) = node.degree == 2 && node.l.constant == false && node.l.feature == 1 && node.r.constant == true && node.op == 4
            num_powers_on_cf = count(g_power_constraint, get_tree(g))
            if num_powers_on_cf > 0
                return 1000 * num_powers_on_cf
            end
    ...
    # Function template
    template = TemplateExpressionSpec("(q() / (q() + pow(U, m()))) * g(C*h(F))",
                                      expressions=["q", "m", "g", "h"],
                                      variable_names=["F", "C", "U"],
    )
