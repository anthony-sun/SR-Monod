...
def main(args):
    ...
    custom_loss = """
        function expr_loss(ex::AbstractExpression, dataset::Dataset, options)
            ...
            num_negative_constants = count(is_negative_constant, get_tree(q)) + count(is_negative_constant, get_tree(m)) + count(is_negative_constant, get_tree(g))
            ...
            num_powers_on_c1 = count(powers_on_c1, get_tree(g))
            ...
            num_powers_on_c2 = count(powers_on_c2, get_tree(g))
    ...
    # Function template
    template = TemplateExpressionSpec("(q() / (q() + pow(U, m()))) * g(F, C1, C2)",
                                      expressions=["q", "m", "g"],
